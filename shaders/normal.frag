...
void main()
{
   ...
   vec3 normal = normalize(texture(normalSampler, inTexCoord).rgb * 2.0 - 1.0);

   vec3 finalColor = vec3(0.0);
   for (int i = 0; i < ubo.lightsCount; i++)
   {
      vec3 lightDir = normalize((TBN * vec3(ubo.lightPositions[i])) -(TBN * inPosition));

      vec3 ambient = ubo.lightColors[i].rgb * 0.5 ;
      vec3 diffuse = (max(0.0,dot(normal, lightDir)) * ubo.lightColors[i].rgb * 0.5 );

      finalColor += ((ambient + diffuse) *texture(baseColorSampler, inTexCoord).rgb);
   }
