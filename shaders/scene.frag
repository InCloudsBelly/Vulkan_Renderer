...
void main()
{
    ...
    for(int i = 0 ; i < ubo.lightsCount; ++i)
    {
        if(lights[i].type == 0)
        {
            Lo += calculateDirLight(i, normal, view, albedo,metallic, roughness,F0);
        }
        else if(lights[i].type == 1)
        {
            Lo += calculatePointLight(i, normal,view, albedo,metallic, roughness,F0);
        }
        else
            Lo += vec3(0.0);
    }
...
vec3 calculateDirLight(int i,vec3 normal,vec3 view,vec3 albedo,float metallic,float roughness,vec3 F0) 
{
...
vec3 calculatePointLight(int i,vec3 normal,vec3 view,vec3 albedo,float metallic,float roughness, vec3 F0) 
{
    return vec3(0.0);
}
